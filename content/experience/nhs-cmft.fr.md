---
date: 2013-09-01
draft: false
title: "Développeur Logiciel Senior"
jobTitle: "Développeur Logiciel Senior"
company: "NHS (CMFT)"
location: "Manchester"
duration: "Sept 2013 - Oct 2014"
---

Le CMFT avait besoin d'un nouveau système de dossiers patients, et aucune application tierce ne répondait à leurs exigences. J'ai donc été recruté en interne pour le développer avec une petite équipe. Nous avons conçu un système basé sur un front-end ASP.NET 4, un back-end WCF et une base de données SQL Server. Les éléments d'interface ont été conçus avec Kendo UI et jQuery, et j'ai ensuite introduit AngularJS pour gérer certains des écrans les plus complexes.

En trois mois, nous avons développé un système entièrement nouveau capable de s'intégrer (via la messagerie HL7) avec d'autres systèmes hospitaliers, doté d'un système de formulaires simple et d'une authentification utilisateur de base. À mon départ, le projet avait évolué jusqu'à permettre aux consultants de visualiser les documents patients dans l'application.
